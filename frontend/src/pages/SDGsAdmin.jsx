...
import { Search, Info, PlusCircle, Edit3, Trash2, X, ImagePlus } from 'lucide-react';
    ...
    return (
        <div className="animate-fade-in space-y-6">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <button 
                    onClick={openAdd}
                    className="bg-emerald-700 text-white px-5 py-2.5 rounded-xl font-bold text-sm flex items-center gap-2 hover:bg-emerald-800 transition-all shadow-sm"
                >
                ...
                <div className="relative w-full sm:w-72">
                    ...
                    <input
                        type="text"
                        placeholder="Cari"
                        className="block w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 transition-all"
                        value={searchTerm}
                            ...
                            {loading ? (
                                [...Array(5)].map((_, i) => (
                                    <tr key={i} className="animate-pulse">
                                        <td colSpan="4" className="px-5 py-6"><div className="h-4 bg-slate-100 rounded w-full"></div></td>
                                    </tr>
                                ))
                                    ...
                                    <tr key={sdg.id} className="hover:bg-slate-50/50 transition-colors group">
                                        <td className="px-5 py-4">
                                            {sdg.gambar ? (
                                                ...
                                                <div 
                                                    ...
                                                    style={{
                                                        backgroundColor: sdg.warna,
                                                        boxShadow: `0 3px 8px -2px ${sdg.warna}50`
                                                    }}
                                                >
                                                    {sdg.no_get}
                                                </div>
                                        ...
                                        <td className="px-5 py-4">
                                            <div className="flex items-center justify-center gap-2">
                                                ...
                                                <button 
                                                    onClick={() => handleDelete(sdg.id)} 
                                                    className="w-9 h-9 rounded-lg bg-rose-50 text-rose-500 flex items-center justify-center hover:bg-rose-100 transition-colors"
                                                    title="Hapus"
                                                >
                    ...
                    <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl animate-scale-up">
                        <div className="p-6 border-b border-slate-100 flex items-center justify-between">
                            <h3 className="text-lg font-black text-slate-800">
                                {editItem ? 'Ubah Tujuan SDGs' : 'Tambah Tujuan SDGs'}
                            </h3>
                            <button onClick={() => setShowModal(false)} className="w-9 h-9 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 hover:text-rose-500 transition-colors">
                                <X className="w-4 h-4" />
                            </button>
                        </div>
                        <form onSubmit={handleSubmit} className="p-6 space-y-5">
                            <div>
                                ...
                                <input 
                                    type="number"
                                    value={formData.no_get} 
                                    onChange={(e) => setFormData({...formData, no_get: parseInt(e.target.value) || ''})}
                                    placeholder="1"
                                    required
                                    className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm font-medium focus:ring-2 focus:ring-indigo-500/10 focus:border-indigo-400 outline-none transition-all"
                                />
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <textarea 
                                    ...
                                    placeholder="Mengakhiri kemiskinan dalam segala bentuk apapun"
                                    rows={3}
                                    className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm font-medium focus:ring-2 focus:ring-indigo-500/10 focus:border-indigo-400 outline-none transition-all resize-none"
                                />
                            </div>
                            <div>
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <button
                                            ...
                                            className="w-full flex items-center gap-3 px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-500 hover:bg-slate-100 transition-all cursor-pointer"
                                        >
                            ...
                            <div className="flex items-center justify-end gap-3 pt-2">
                                <button type="button" onClick={() => setShowModal(false)} className="px-6 py-2.5 text-sm font-bold text-slate-500 hover:text-slate-700 transition-colors">
                                    Batal
                                </button>
                                <button type="submit" className="px-8 py-2.5 bg-emerald-700 text-white rounded-xl font-bold text-sm hover:bg-emerald-800 transition-all shadow-sm">
                                    Simpan
                                </button>
                            </div>
                        </form>
